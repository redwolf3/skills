...
Every skill description **must** be prefixed with `(redwolf3)` — e.g. `(redwolf3) Does something useful.` This applies to both the `description:` field in `SKILL.md` frontmatter and to all references in `README.md` files and `plugin.json`.

Skills in `engineering/` and `productivity/` also have a human-facing docs page at `docs/<bucket>/<skill-name>.md`. When you add, rename, or change the behaviour of a skill in `engineering/` or `productivity/`, create or re-sync its docs page. Skills in the non-promoted buckets (`misc/`, `personal/`, `in-progress/`, `deprecated/`) get **no** docs page.
